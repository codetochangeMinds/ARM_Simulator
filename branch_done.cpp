...
void EX()
{
	...
	{	
		...
		else if(Register_ID_EX.opcode > 6 && Register_ID_EX.opcode < 10)
		{
...
make_instruction_file();
   ...
   /* for (int i = 0; i < instruction_file.size(); i++)
    ...
    while(shut_down == 0 && end_file_inst == 0)
    {
    	WB();
    	MEM();
    	//cout<<Register_MEM_WB.opcode<<" "<<Register_MEM_WB.type<<" "<<Register_MEM_WB.write_data<<" "<<Register_MEM_WB.src<<" "<<" ||| ";
    	if(PCSrc == 0)
    	{	EX();
